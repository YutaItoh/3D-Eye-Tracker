...
float singleeyefitter::cvx::histKmeans(const cv::Mat_<float>& hist, int bin_min, int bin_max, int K, float init_centres[], cv::Mat_<uchar>& labels, cv::TermCriteria termCriteria)
{
    ...
    size_t nbins = hist.total();
    float binWidth = (bin_max - bin_min)/(float)nbins;
    float binStart = bin_min + binWidth/2.0f;
...
cv::RotatedRect singleeyefitter::cvx::fitEllipse(const cv::Moments& m)
{
    ...
    ret.center.x = (float)(m.m10/m.m00);
    ret.center.y = (float)(m.m01/m.m00);
    ...
    ret.size.width = (float)std::sqrt(2*(mu20 + mu02 + common));
    ret.size.height = (float)std::sqrt(2*(mu20 + mu02 - common));
    ...
    if (num == 0 && den == 0)
        ...
        ret.angle = (float)(180/PI * std::atan2(num,den));
...
{
    return cv::Vec2f(
		ellipse.size.width*(float)std::cos(PI / 180.0 * (double)ellipse.angle),
		ellipse.size.width*(float)std::sin(PI / 180.0 * (double)ellipse.angle));
}
